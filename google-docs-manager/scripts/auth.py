#!/usr/bin/env python3
"""
Google Docs Service Account authentication.
...
def build_docs_service(credentials_path: str = DEFAULT_CREDENTIALS_PATH):
    """Build and return an authenticated Google Docs API service."""
    creds = load_credentials(credentials_path)
    return build("docs", "v1", credentials=creds)


def build_drive_service(credentials_path: str = DEFAULT_CREDENTIALS_PATH):
    """Build and return an authenticated Google Drive API service (for search)."""
    creds = load_credentials(credentials_path)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Google Docs Service Account auth management")
